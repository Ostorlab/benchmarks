...
object AuthManager {
    ...
    fun getAuthorizationUrl(context: Context): String {
        ...
        val codeVerifier = Base64.encodeToString(codeVerifierBytes, Base64.URL_SAFE or Base64.NO_WRAP or Base64.NO_PADDING)
        ...
        return "$AUTHORIZE_URL?response_type=code&client_id=$CLIENT_ID&redirect_uri=$encodedRedirectUri&state=$state&code_challenge=$codeChallenge&code_challenge_method=$codeChallengeMethod"
    }
    ...
    suspend fun exchangeCodeForToken(context: Context, code: String, state: String): Boolean = withContext(Dispatchers.IO) {
        ...
        if (codeVerifier == null) {
            Log.e("AuthManager", "Code verifier not found.")
            return@withContext false
        }

        // Clear the state and verifier after use
        prefs.edit {
            remove(KEY_STATE)
