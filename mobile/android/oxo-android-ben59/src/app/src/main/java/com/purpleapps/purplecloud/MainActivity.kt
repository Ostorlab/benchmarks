...
import com.purpleapps.purplecloud.cloud.LocalWebServer
import java.io.File
...
class MainActivity : ComponentActivity() {
    ...
    private var authCodeState = mutableStateOf<Pair<String, String>?>(null)
    ...
    private fun handleIntent(intent: Intent) {
        if (intent.action == Intent.ACTION_VIEW) {
            val uri = intent.data
            if (uri != null && uri.toString().startsWith("com.purpleapps.purplecloud://oauth2/callback")) {
                val code = uri.getQueryParameter("code")
                val state = uri.getQueryParameter("state")
                if (code != null && state != null) {
                    authCodeState.value = Pair(code, state)
                }
            }
        }
    }
                ...
                if (hasPermission) {
                    ...
                    if (isLoggedIn) {
                        ...
                        LoginScreen(
                            ...
                            onLoginSuccess = {
                                isLoggedIn = true
                                authCodeState.value = null // Reset after use
                            }
                        )
                    }
                } else {
...
fun MainScreen(modifier: Modifier = Modifier, userEmail: String, onLogout: () -> Unit) {
    ...
    // State for Wi-Fi sharing
    var isWifiSharingOn by remember { mutableStateOf(false) }
    var wifiSharingInfo by remember { mutableStateOf<Pair<String?, String?>>(null to null) }
    val localWebServer = remember(context) { LocalWebServer(context) }

    // Ensure server is stopped when the screen is disposed
    ...
    Scaffold(
        ...
        topBar = {
            Column {
                TopAppBar(
                    ...
                    actions = {
                        Icon(
                            ...
                            contentDescription = "Wi-Fi Sharing",
                            tint = MaterialTheme.colorScheme.primary
                        )
                        Switch(
                            checked = isWifiSharingOn,
                            onCheckedChange = { isOn ->
                                isWifiSharingOn = isOn
                                if (isOn) {
                                    wifiSharingInfo = localWebServer.start()
                                } else {
                                    localWebServer.stop()
                                    wifiSharingInfo = null to null
                                }
                            }
                        )
                ...
                if (isWifiSharingOn) {
                    val (url, code) = wifiSharingInfo
                    val urlText = url ?: "Starting server..."
                    Text(
                        text = urlText,
                        modifier = Modifier
 ...
 *
 * @param authCode The authorization code and state received from the OAuth redirect.
 * @param onLoginSuccess The callback to be invoked when the user successfully logs in.
 */
@Composable
fun LoginScreen(authCode: Pair<String, String>?, onLoginSuccess: () -> Unit) {
    val context = LocalContext.current
    var error by remember { mutableStateOf<String?>(null) }
    var isLoading by remember { mutableStateOf(false) }
    val scope = rememberCoroutineScope()

    // If we receive an auth code, exchange it for a token.
    LaunchedEffect(authCode) {
        if (authCode != null) {
            isLoading = true
            val (code, state) = authCode
            val success = AuthManager.exchangeCodeForToken(context, code, state)
            if (success) {
                onLoginSuccess()
            } else {
    ...
    Column(
        ...
        if (isLoading) {
            ...
            Button(onClick = {
                ...
                Text(stringResource(R.string.login_with_purplecloud))
            }
        }
