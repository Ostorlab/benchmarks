...
object CloudManager {
    ...
    suspend fun listDirectory(context: Context, path: String): List<FileSystemItem> = withContext(Dispatchers.IO) {
        var token = AuthManager.getAccessToken(context)
        if (token == null) {
        ...
        try {
            ...
            fun makeRequest(accessToken: String): HttpURLConnection {
            ...
            var connection = makeRequest(token)

            if (connection.responseCode == HttpURLConnection.HTTP_UNAUTHORIZED) {
                Log.d(TAG, "Access token may have expired, attempting to refresh.")
                val newAccessToken = AuthManager.refreshAccessToken(context)
                if (newAccessToken != null) {
                    token = newAccessToken
                    connection.disconnect()
                    connection = makeRequest(token)
                } else {
                    Log.e(TAG, "Failed to refresh access token. Cannot list directory.")
        ...
        try {
            ...
            if (connection.responseCode == HttpURLConnection.HTTP_UNAUTHORIZED) {
                ...
                if (newAccessToken != null) {
                    ...
                    Log.e(TAG, "Failed to refresh access token. Cannot delete item.")
                    return@withContext false
                }
            }
     ...
     */
    private suspend fun uploadFile(context: Context, file: LocalFile, logicalPath: String) {
        ...
        try {
            ...
            fun writeBody(connection: HttpURLConnection) {
            ...
            fun makeConnection(accessToken: String): HttpURLConnection {
                val connection = url.openConnection() as HttpURLConnection
            ...
            var connection = makeConnection(token)
            ...
            if (responseCode == HttpURLConnection.HTTP_UNAUTHORIZED) {
                ...
                if (newAccessToken != null) {
                    ...
                    Log.e(TAG, "Failed to refresh access token. Cannot upload file.")
                    connection.disconnect()
                    return
                }
            }
