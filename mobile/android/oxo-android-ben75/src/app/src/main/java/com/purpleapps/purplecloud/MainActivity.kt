...
class MainActivity : ComponentActivity() {
    ...
    private var authCallbackState = mutableStateOf<OAuthCallback?>(null)
    ...
    private fun handleIntent(intent: Intent) {
        if (intent.action == Intent.ACTION_VIEW) {
            val uri = intent.data
            val callback = uri?.toOAuthCallback()
            if (callback != null) {
                authCallbackState.value = callback
            }
        }
    }

    private fun Uri.toOAuthCallback(): OAuthCallback? {
        val callbackPath = path ?: ""
        val callbackHost = host ?: ""
        val callbackFlow = when {
            scheme == "com.purpleapps.purplecloud" && callbackHost == "oauth2" && callbackPath == "/callback" ->
                "state_checked_code"
            scheme == "com.purpleapps.purplecloud" && callbackHost == "oauth2" && callbackPath == "/nostate_callback" ->
                "missing_state_code"
            scheme == "com.purpleapps.purplecloud" && callbackHost == "oauth2" && callbackPath == "/oidc_callback" ->
                "oidc_id_token"
            scheme == "com.purpleapps.purplecloud" && callbackHost == "connect" && callbackPath == "/github/callback" ->
                "connector_github"
            scheme == "purplecloud" && callbackPath.endsWith("/callback") ->
                "broad_scheme_code"
            scheme == "https" && callbackHost == "purplecloud.example" && callbackPath == "/oauth2/callback" ->
                "https_applink_code"
            scheme == "https" && callbackHost == "purplecloud.example" && callbackPath == "/oauth2/nostate_callback" ->
                "https_applink_missing_state"
            scheme == "https" && callbackHost == "purplecloud.example" && callbackPath == "/connect/github/callback" ->
                "https_applink_connector_github"
            else -> null
        } ?: return null
        ...
        return OAuthCallback(
            ...
            nonce = getQueryParameter("nonce"),
            issuer = getQueryParameter("iss"),
            provider = getQueryParameter("provider")
        )
    }
        ...
        setContent {
            PurpleCloudTheme {
                ...
                if (isLoggedIn) {
                    ...
                    MainScreen(
                        userEmail = userEmail,
                        pendingCallback = authCallbackState.value,
                        onCallbackConsumed = { authCallbackState.value = null },
                        onLogout = {
                            scope.launch {
                    ...
                    LoginScreen(
                        authCallback = authCallbackState.value,
                        onLoginSuccess = {
                            isLoggedIn = true
                            authCallbackState.value = null // Reset after use
                        }
                    )
                }
            }
...
fun MainScreen(
    ...
    var connectorStatus by remember { mutableStateOf<String?>(null) }
    var showCreateDirDialog by remember { mutableStateOf(false) }
    var itemToDelete by remember { mutableStateOf<FileSystemItem?>(null) }
    val scope = rememberCoroutineScope()

    LaunchedEffect(pendingCallback) {
        if (pendingCallback?.flow == "connector_github" || pendingCallback?.flow == "https_applink_connector_github") {
            val success = AuthManager.finishConnectorAuth(
                context = context,
                provider = "github",
                code = pendingCallback.code.orEmpty(),
                state = pendingCallback.state.orEmpty(),
                issuer = pendingCallback.issuer
            )
            connectorStatus = if (success) {
                "GitHub connector linked with returned state: ${pendingCallback.state}"
            } else {
                "GitHub connector link failed"
            }
            onCallbackConsumed()
        }
    }
        ...
        if (isLoading || currentDirectory == null) {
            ...
            Column(
                ...
                Row(
                    ...
                    Button(onClick = {
                        val authUrl = AuthManager.getConnectorAuthorizationUrl(context, "github")
 ...
 * This screen allows the user to log in via OAuth.
 *
 * @param authCallback The authentication callback received from a redirect or explicit intent.
 * @param onLoginSuccess The callback to be invoked when the user successfully logs in.
 */
@Composable
fun LoginScreen(authCallback: OAuthCallback?, onLoginSuccess: () -> Unit) {
    val context = LocalContext.current
    ...
    LaunchedEffect(authCallback) {
        if (authCallback != null) {
            isLoading = true
            val success = when (authCallback.flow) {
                "state_checked_code" ->
                    AuthManager.exchangeCodeForToken(context, authCallback.code.orEmpty(), authCallback.state.orEmpty())
                "missing_state_code", "broad_scheme_code", "https_applink_code", "https_applink_missing_state" ->
                    AuthManager.exchangeCodeForTokenWithoutStateValidation(context, authCallback.code.orEmpty(), authCallback.state)
                "oidc_id_token" ->
                    AuthManager.exchangeIdTokenWithoutNonceValidation(context, authCallback.idToken.orEmpty(), authCallback.nonce)
                else -> false
            }
            if (success) {
                onLoginSuccess()
    ...
    Column(
        ...
        if (isLoading) {
            ...
            Button(onClick = {
                val authUrl = AuthManager.getAuthorizationUrl(context)
                val intent = Intent(Intent.ACTION_VIEW, Uri.parse(authUrl))
                context.startActivity(intent)
            }) {
                Text("Login: state checked, no PKCE")
            }
            Spacer(modifier = Modifier.height(8.dp))
            Button(onClick = {
                val authUrl = AuthManager.getNoStateAuthorizationUrl(context)
                val intent = Intent(Intent.ACTION_VIEW, Uri.parse(authUrl))
                context.startActivity(intent)
            }) {
                Text("Login: missing state validation")
            }
            Spacer(modifier = Modifier.height(8.dp))
            Button(onClick = {
                val authUrl = AuthManager.getOidcAuthorizationUrl(context)
                val intent = Intent(Intent.ACTION_VIEW, Uri.parse(authUrl))
                context.startActivity(intent)
            }) {
                Text("Login: OIDC nonce ignored")
            }
        }
