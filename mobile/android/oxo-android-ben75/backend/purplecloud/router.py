...
def _get_login_form(client_id, redirect_uri, response_type, state, error=""):
...
async def authorize(
    ...
    if request.method == "POST":
        ...
        if not user:
            html_form = _get_login_form(
                client_id, redirect_uri, response_type, state, "Invalid credentials"
            )
            return responses.HTMLResponse(content=html_form, status_code=401)

        code = auth.create_authorization_code(username=user.username, client_id=client_id)
    ...
    if redirect_uri not in auth.REGISTERED_CLIENTS[client_id]["redirect_uris"]:
        raise fastapi.HTTPException(400, "Mismatched redirect_uri.")
    if response_type != "code":
        raise fastapi.HTTPException(400, "Unsupported response_type.")

    html_form = _get_login_form(client_id, redirect_uri, response_type, state)
...
async def oidc_authorize(
    ...
    username: str = Query("testuser@example.com"),
):
    """
    Native OIDC-style authorization endpoint.

    This intentionally returns an unsigned id_token through the mobile redirect to
    exercise client/backend nonce and ID token validation tests.
    """
    if client_id not in auth.REGISTERED_CLIENTS:
        raise fastapi.HTTPException(400, "Invalid client_id.")
    if redirect_uri not in auth.REGISTERED_CLIENTS[client_id]["redirect_uris"]:
    ...
    if username not in auth.fake_users_db:
        raise fastapi.HTTPException(400, "Unknown test user.")

    id_token = auth.create_unsigned_id_token(username, client_id, nonce)
    params = {
        "id_token": id_token,
        "nonce": nonce,
        "iss": "https://purplecloud.example",
    }
    return responses.RedirectResponse(f"{redirect_uri}?{urlencode(params)}")


@router.post("/oidc/native_login", response_model=auth.OAuthToken)
async def oidc_native_login(
    id_token: str = Form(...),
    client_id: str = Form(...),
    nonce: Optional[str] = Form(None),
):
    """Deliberately accepts an unsigned id_token and ignores nonce binding."""
    tokens = auth.login_with_unvalidated_id_token(id_token, client_id, nonce)
    if not tokens:
        raise fastapi.HTTPException(401, "Invalid id_token.")
    access_token, refresh_token = tokens
    return auth.OAuthToken(access_token=access_token, refresh_token=refresh_token)


@router.get("/connect/{provider}/authorize")
async def connector_authorize(
    provider: str,
    client_id: str = Query(None),
    redirect_uri: str = Query(None),
    state: str = Query(None),
    attacker: str = Query("attacker@example.com"),
):
    """Simulate a third-party connector OAuth authorization redirect."""
    if client_id not in auth.REGISTERED_CLIENTS:
        raise fastapi.HTTPException(400, "Invalid client_id.")
    if redirect_uri not in auth.REGISTERED_CLIENTS[client_id]["redirect_uris"]:
        raise fastapi.HTTPException(400, "Mismatched redirect_uri.")
    code = auth.create_authorization_code(username=attacker, client_id=client_id)
    params = {
        "code": code,
        "state": state,
        "iss": f"https://{provider}.example",
        "provider": provider,
    }
    return responses.RedirectResponse(f"{redirect_uri}?{urlencode(params)}")


@router.post("/connect/{provider}/finish")
async def connector_finish(
    provider: str,
    ...
    """Deliberately links a provider account without validating connector state."""
    linked = auth.link_provider_account(
...
async def token(
    ...
    if grant_type == "authorization_code":
        ...
        tokens = auth.exchange_code_for_tokens(code, client_id, redirect_uri)
        if not tokens:
...
@router.get("/debug/provider_links")
async def debug_provider_links():
    """Return linked providers to simplify before/after impact validation."""
    return auth._read_tokens(auth.PROVIDER_LINKS_FILE)
