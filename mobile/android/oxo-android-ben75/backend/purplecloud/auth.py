...
PROVIDER_LINKS_FILE = TOKEN_DIR / "provider_links.json"
    ...
    with _lock:
        ...
        if isinstance(tokens, dict):
            ...
            active_tokens = {
                ...
                if not isinstance(data, dict) or "expiry" not in data or data["expiry"] > now
            }
...
fake_users_db = {
    ...
    "attacker@example.com": {
        ...
        "hashed_password": pwd_context.hash("attackerpass"),
        "disabled": False,
    },
    "victim@example.com": {
        ...
        "hashed_password": pwd_context.hash("victimpass"),
        "disabled": False,
    }
}
...
def create_authorization_code(username: str, client_id: str) -> str | None:
    ...
    if isinstance(auth_codes, dict):
        auth_codes[code] = {"username": username, "client_id": client_id, "expiry": expiry}
        _write_tokens(AUTH_CODES_FILE, auth_codes)
        return code
    return None


def create_unsigned_id_token(username: str, client_id: str, nonce: str | None) -> str:
    """Create a deliberately unsigned ID token for OIDC test coverage."""
...
def login_with_unvalidated_id_token(
    id_token: str, client_id: str, returned_nonce: str | None
) -> tuple[str, str] | None:
    """Deliberately trust an unsigned ID token and ignore nonce binding."""
    try:
    ...
    # VULNERABLE: no signature validation, no nonce claim comparison, no strict issuer validation.
    ...
    if username not in fake_users_db:
        fake_users_db[username] = {
            ...
            "email": username,
            "hashed_password": pwd_context.hash(secrets.token_urlsafe(12)),
            "disabled": False,
        }
    return create_auth_tokens(username=username)
...
def exchange_code_for_tokens(
    ...
    if code_data["client_id"] != client_id or client_id not in REGISTERED_CLIENTS:
        return None
    # Verify redirect_uri matches a registered one. Some registered entries are intentionally
    # weak to exercise mobile callback tests.
...
def link_provider_account(username: str, provider: str, code: str, state: str, issuer: str | None):
    """Deliberately link a provider account without verifying a stored connector state."""
    links = _read_tokens(PROVIDER_LINKS_FILE)
    if not isinstance(links, dict):
        links = {}
    user_links = links.setdefault(username, {})
    user_links[provider] = {
        "provider_subject": f"{provider}:{code[:12]}",
        "code": code,
        "state": state,
        "issuer": issuer,
        "linked_at": time.time(),
    }
    _write_tokens(PROVIDER_LINKS_FILE, links)
    return user_links[provider]
