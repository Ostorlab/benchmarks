...
from purplecloud.auth import generate_2fa_code, verify_2fa_code, clear_2fa_code
...
def _get_2fa_form(
    ...
    """Generates the HTML form for 2FA code entry."""
    error_html = f"<p style='color: red;'>{error}</p>" if error else ""
    html_content = f"""
    <html>
        ...
        <body>
            ...
            <form method="post">
                ...
                <label for="2fa_code">2FA Code:</label><br>
                <input type="text" id="2fa_code" name="2fa_code" pattern="\\d{{4}}"><br><br>
...
async def authorize(
    ...
    if request.method == "POST":
        ...
        tfa_code = form_data.get("2fa_code")

        # If this is the 2FA step, we only need the username from the form.
        # Otherwise, we need to authenticate with password first.
        if not tfa_code:
            ...
            if user.two_factor_enabled:
                auth.generate_2fa_code(user.username)
                return _get_2fa_form(
                    client_id,
                    redirect_uri,
                    response_type,
                    state,
            ...
            user = auth.get_user(auth.fake_users_db, username)
            ...
            if not auth.verify_2fa_code(user.username, tfa_code):
                # VULNERABILITY: Allows infinite guesses by showing the form again
                return _get_2fa_form(
                    client_id,
                    redirect_uri,
                    response_type,
            ...
            auth.clear_2fa_code(user.username)
