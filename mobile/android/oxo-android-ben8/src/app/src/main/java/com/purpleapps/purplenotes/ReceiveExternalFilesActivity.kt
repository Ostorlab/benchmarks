 ...
 * adb shell am start-activity -n com.ostorlab.benchmark.vulnreceiver/.ReceiveExternalFilesActivity \
 *   -a com.ostorlab.benchmark.vulnreceiver.action.EXPORT_NOTE \
 *   --es note_name "../../shared_prefs/com.ostorlab.benchmark.vulnreceiver_preferences.xml"
 */
class ReceiveExternalFilesActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            if (noteFile.exists() && noteFile.isFile) {
                ...
                Log.i("ReceiveExternalFilesActivity",
                    "Sending back file content of the note. $content"
                )
                setResult(RESULT_OK, resultIntent)
            } else {
                Log.i("ReceiveExternalFilesActivity", "Note file does not exist or is not a file.")
                setResult(RESULT_CANCELED)
            }
        } catch (e: Exception) {
            Log.e("ReceiveExternalFilesActivity", "Error exporting note", e)
            setResult(RESULT_CANCELED)
        }
